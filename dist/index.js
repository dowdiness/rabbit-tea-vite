...
function probeMoonBitPackage(sourceRoot, modConfig) {
    const pkgConfigs = [];
    const worklist = [sourceRoot];
...
function probeMoonBitModule() {
    ...
    if (fs.existsSync(modJsonPath)) {
        ...
        const pkgConfigs = probeMoonBitPackage(path.join(cwd, modConfig.source), modConfig);
...
export default function rabbitTEA(mainPackagePath) {
    const { modConfig, pkgConfigs } = probeMoonBitModule();
    var mainPkgConfig = undefined;
    ...
    return {
        ...
        handleHotUpdate({ server, modules, timestamp }) {
            const mods = modules.filter(({ file }) => !(file?.endsWith('.js') && file == path.resolve(tempJsPath)
